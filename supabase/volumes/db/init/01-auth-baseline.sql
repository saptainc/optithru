-- 02-auth-baseline.sql
-- AUTO-GENERATED — do not edit by hand. Regenerate with:
--   docker compose exec -T db pg_dump -U supabase_admin -h localhost -d postgres \
--     --schema=auth --schema-only > schema.sql
--   docker compose exec -T db pg_dump -U supabase_admin -h localhost -d postgres \
--     --table='auth.schema_migrations' --data-only --inserts > data.sql
--
-- Why this exists: supabase/postgres v15.8.1.060 no longer pre-populates the
-- auth schema baseline that supabase/gotrue v2.164.0 expects. The GoTrue
-- migrations only ADD to a pre-existing baseline; they don't create the base
-- schema (factor_type enum, users table, etc.) from scratch. Without this
-- baseline, GoTrue fails on `20240729123726_add_mfa_phone_config.up.sql` with:
--
--   ERROR: type "auth.factor_type" does not exist
--
-- This file dumps the working auth schema from a healthy production deploy
-- and ships it as a first-init script. After it loads, GoTrue sees all
-- migrations as already applied (via the schema_migrations data) and starts
-- up cleanly with no further migrations to run.
--
-- Drop and recreate the auth schema before applying the baseline. This is
-- safe because init scripts only run when the postgres data dir is empty.
...
-- ── Schema migrations data (mark all baseline migrations as applied) ──
--
-- PostgreSQL database dump
--
